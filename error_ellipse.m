...
% make sure covariance matrix is 2x2 or 3x3
[r,c] = size(C);
if r ~= c | (r ~= 2 & r ~= 3)
  error(['Don''t know what to do with ',num2str(r),'x',num2str(c),' matrix'])
end

% center ellipse around supplied mean values
x0=mu(1);
y0=mu(2);
...
if r==3 & c==3
  ...
  % tilt the ellipsoid to line up with the data
  XYZ = [X(:),Y(:),Z(:)]*sqrt(eigval)*eigvec';
  
  % scale is a user-defined scaling factor (eg to convert from m to km)
  % k fixes the confidence level and is calculated from chi squared dist
  % x0, y0, z0 center the ellipse at the user-defined values
  X(:) = scale*(k*XYZ(:,1)+x0);
  Y(:) = scale*(k*XYZ(:,2)+y0);
  Z(:) = scale*(k*XYZ(:,3)+z0);
  h4=surf(X,Y,Z);
